...
public class ProfileListArrayAdapter extends ArrayAdapter<Profile> {
    ...
    private final ArrayList<Event> eventList; // Shared list of events
    private OrganizerEventArrayAdapter eventsAdapter; // Reference to the events adapter

    private final ArrayList<Facility> facilities; // Shared list of facilities
     ...
     */
    public static void deleteProfileFromDatabase(Context context, FirebaseFirestore db,
                                                 Profile profile,
                                                 FacilityListArrayAdapter facilitiesAdapter,
                                                 OrganizerEventArrayAdapter eventsAdapter) {
        // Has Profile Picture
        if (!profile.getUsingDefaultPicture()) {
            AdminImagesAdapter.deleteImageFromFirestore(context, db,
                                                        profile.getProfilePicturePath(), false);
        }
        ...
        deleteProfileFromDatabaseOrganizer(context, db, profile.getDeviceId(), true,
                                           facilitiesAdapter, eventsAdapter);
     ...
     */
    private static void deleteProfileFromDatabaseEntrant(Context context, FirebaseFirestore db,
                                                         String deviceId) {
        db.collection("entrants").document(deviceId).get()
          .addOnSuccessListener(documentSnapshot -> {
              if (documentSnapshot.exists()) {
                  // Fetch event lists from the entrant document
                  ArrayList<String> waitlistedEvents = (ArrayList<String>) documentSnapshot.get(
                          "waitlistedEvents");
                  ArrayList<String> selectedEvents = (ArrayList<String>) documentSnapshot.get(
                          "invitedEvents");
                  ArrayList<String> finalistEvents = (ArrayList<String>) documentSnapshot.get(
                          "finalistEvents");
                  ArrayList<String> cancelledEvents = (ArrayList<String>) documentSnapshot.get(
                          "uninvitedEvents");
                  ...
                  db.collection("entrants").document(deviceId).delete()
                    .addOnSuccessListener(aVoid -> {
                        Toast.makeText(context, "Entrant data deleted successfully.",
                                       Toast.LENGTH_SHORT).show();
                    })
                    .addOnFailureListener(e -> {
                        Toast.makeText(context, "Failed to delete entrant data: " + e.getMessage(),
                                       Toast.LENGTH_SHORT).show();
                        e.printStackTrace();
                    });
              } else {
          ...
          .addOnFailureListener(e -> {
              Toast.makeText(context, "Failed to fetch entrant profile: " + e.getMessage(),
                             Toast.LENGTH_SHORT).show();
              e.printStackTrace();
          });
    }
     ...
     */
    private static void deleteProfileFromDatabaseOrganizer(Context context, FirebaseFirestore db,
                                                           String deviceId, boolean FromProfile,
                                                           FacilityListArrayAdapter facilitiesAdapter,
                                                           OrganizerEventArrayAdapter eventsAdapter) {
          ...
          .addOnSuccessListener(documentSnapshot -> {
              if (documentSnapshot.exists()) {
                  ...
                  if (facility.getDeviceId() != null && !facility.getDeviceId().isEmpty()) {
                      ...
                      FacilityListArrayAdapter.showFacilityOptionsDialog(context, db, facility,
                                                                         eventsAdapter,
                                                                         facilitiesAdapter,
                                                                         FromProfile);
                  } else {
                      Toast.makeText(context, "No associated facility found for organizer.",
                                     Toast.LENGTH_SHORT).show();
                  }
              } else {
                  Toast.makeText(context, "Organizer profile not found.", Toast.LENGTH_SHORT)
                       .show();
              }
          })
          .addOnFailureListener(e -> {
              Toast.makeText(context, "Failed to fetch organizer profile: " + e.getMessage(),
                             Toast.LENGTH_SHORT).show();
              e.printStackTrace();
          });
    }

    public static void EntrantRemovalWaitlisted(FirebaseFirestore db,
                    ...
                    .addOnSuccessListener(documentSnapshot -> {
                        if (documentSnapshot.exists()) {
                            List<Map<String, List<Double>>> joinLocations =
                                    (List<Map<String, List<Double>>>) documentSnapshot.get(
                                            "joinLocations");
                            ...
                            if (joinLocations != null) {
                                ...
                                for (Map<String, List<Double>> entry : joinLocations) {
                                    if (!entry.containsKey(deviceId)) {
                                        updatedJoinLocations.add(
                                                entry); // Keep entries not matching the deviceId
                                    }
                                }
                                ...
                                db.collection("events").document(eventID)
                                  ...
                                  .addOnSuccessListener(aVoid2 -> System.out.println(
                                          "Device ID removed from joinLocations: " + eventID))
                                  .addOnFailureListener(e -> System.err.println(
                                          "Failed to update joinLocations for event: " + eventID +
                                          " - " + e.getMessage()));
                            }
                        }
                    })
                    .addOnFailureListener(e -> System.err.println(
                            "Failed to fetch joinLocations for event: " + eventID + " - " +
                            e.getMessage()));
              })
              .addOnFailureListener(e -> System.err.println(
                      "Failed to remove Device ID from event lists: " + eventID + " - " +
                      e.getMessage()));
        }
    }

    public static void EntrantRemovalSelected(FirebaseFirestore db,
                                              ArrayList<String> SelectedEvents, String deviceId) {
    ...
    public static void EntrantRemovalFinalist(FirebaseFirestore db,
                                              ArrayList<String> FinalistEvents, String deviceId) {
    ...
    public static void EntrantRemovalCancelled(FirebaseFirestore db,
                                               ArrayList<String> CancelledEvents, String deviceId) {
        ...
        for (String eventID : CancelledEvents) {
            db.collection("events").document(eventID)
              ...
              .addOnFailureListener(e -> {
                  ...
                  System.err.println("Failed to remove Device ID from event: " + eventID + " - " +
                                     e.getMessage());
              });
        }
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = LayoutInflater.from(getContext())
                                        .inflate(R.layout.content_admin_profiles, parent, false);
        }
        if (eventsAdapter == null) {
            eventsAdapter = new OrganizerEventArrayAdapter(context, eventList, true);
        }
        if (facilitiesAdapter == null) {
            facilitiesAdapter = new FacilityListArrayAdapter(context, facilities, isAdmin,
    ...
    private void showProfileOptionsDialog(Profile profile) {
        ...
        if (!profile.getUsingDefaultPicture()) {
            ...
            AdminActivity.showAdminAlertDialog(context, () ->
                                                       AdminActivity.showAdminAlertDialog(context,
                                                                                          () -> deleteProfileFromDatabase(context, db,
                                                                                                                          profile,
                                                                                                                          facilitiesAdapter,
                                                                                                                          eventsAdapter),
                                                                                          "Confirm Deletion",
