...
public class GeolocationMapsActivity extends AppCompatActivity implements OnMapReadyCallback {
        ...
        db.collection("events").whereEqualTo("eventID", eventID)
          ...
          .addOnCompleteListener(task -> {
              if (task.isSuccessful() && !task.getResult().isEmpty()) {
                  ...
                  joinLocations = (ArrayList<Map<String, List<Double>>>) documentSnapshot.get(
                          "joinLocations");
    ...
    private double calculateDistanceBetweenTwoLatLng(LatLng latLng1, LatLng latLng2) {
        ...
        Location.distanceBetween(latLng1.latitude, latLng1.longitude, latLng2.latitude,
                                 latLng2.longitude, results);
        return (double) results[0] /
               1000.0; // Convert to km since we need to compare with radius but cast to double for extra precision
    }
